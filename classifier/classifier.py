...
def get_special_features(path='raw_features'):
    classes = _ALL_CLASSES
    pattern = '({}).*(\.csv)'.format('|'.join(classes))
    reg = re.compile(pattern, re.IGNORECASE)
    for f in os.listdir(path):
        m = reg.search(f)
        if not m: continue
        c = m.groups()[0].lower()
        feat = np.genfromtxt(os.path.join(path,f), delimiter=',')
        print("{}: Min {} max {}, length {}".format(c, np.min(feat), np.max(feat), len(feat)))
    return
